  ...
  return (
    ...
    >
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4">
            ...
            >
              <div
                className={`max-w-xl rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                  msg.role === "human"
                    ? "bg-white/70 border border-[#649E97]/25 text-[#2D4A40] rounded-br-sm"
                    : "bg-white/65 border border-[#649E97]/25 backdrop-blur-md text-[#0A2E22] rounded-bl-sm"
                }`}
              >
                {msg.role === "human" ? (
                  msg.content
          ...
          {sending && !messages.some((m) => m.id === "streaming") && (
            <div className="flex justify-start">
              <div className="max-w-xl rounded-2xl px-4 py-3 bg-white/65 border border-[#649E97]/25 backdrop-blur-md rounded-bl-sm">
        ...
        <div className="flex-shrink-0 border-t border-[#649E97]/15 bg-white/75 backdrop-blur-xl px-6 py-4">
          <div className="flex items-end gap-3">
            <textarea
              ...
              className="flex-1 resize-none rounded-xl border border-[#649E97]/30 bg-white px-4 py-2.5 text-sm/6 text-gray-900 placeholder:text-[#649E97]/50 shadow-sm focus:outline-2 focus:-outline-offset-2 focus:outline-[#006B4E]/40 disabled:opacity-50"
            />
            <button
              onClick={() => handleSend()}
              disabled={sending || !input.trim()}
              className="inline-flex items-center rounded-lg border border-[#649E97]/30 bg-white px-4 py-2.5 text-sm/6 font-semibold text-[#2D4A40] hover:bg-[#649E97]/10 disabled:opacity-50 transition-colors"
            >
              {sending ? "…" : "Send"}
            </button>
          </div>
